...
const index = require("../index-CA3neWUU.js");
...
class MxCadAssetsWebpackPlugin {
  ...
  forceConfigureDevServer(compiler, logger) {
    ...
    if (hasSetupMiddlewares) {
      ...
      const defaultPublicDir = path__namespace.join(context, "public", this.options.outputDir);
      ...
      devServerConfig.setupMiddlewares = (middlewares, server) => {
        ...
        server.app.use(basePath, index.createMxCadAssetsMiddleware(this.sourceAssetsPath, this.options, defaultPublicDir, logger));
  ...
  injectMiddlewareForWDSv3(compiler, basePath, logger) {
    ...
    if (devServer) {
      if (devServer.server && devServer.server.app) {
        ...
        devServer.server.app.use(basePath, index.createMxCadAssetsMiddleware(thisOptions.sourceAssetsPath, thisOptions.options, defaultPublicDir, logger));
      }
      ...
      devServer.listen = function(...args) {
        ...
        this.on("listening", () => {
          if (this.app && !this.app._mxcad_injected) {
            ...
            this.app.use(basePath, index.createMxCadAssetsMiddleware(thisOptions.sourceAssetsPath, thisOptions.options, defaultPublicDir, logger));
        ...
        Server.prototype.setupMiddlewares = function(...args) {
          if (!this._mxcad_injected) {
            ...
            this.options.setupMiddlewares = (middlewares, server) => {
              ...
              this.app.use(index.createGlobalUniversalMiddleware(thisOptions.options));
              server.app.use(basePath, index.createMxCadAssetsMiddleware(thisOptions.sourceAssetsPath, thisOptions.options, defaultPublicDir));
  ...
  copyAssetsToOutput(compiler, logger, callback) {
    ...
    outputFs.mkdir(targetAssetsPath, { recursive: true }, (err) => {
      ...
      const context = compiler.options.context || process.cwd();
      const defaultPublicDir = path__namespace.join(context, "public", this.options.outputDir);
      index.copyDir(this.sourceAssetsPath, targetAssetsPath, defaultPublicDir, this.options, (err2) => {
        if (err2) {
