...
import { r as resolveOptions, g as getSourceAssetsPath, a as getExternals, b as createGlobalUniversalMiddleware, c as copyDir, i as generateRuntimeConfigScript, d as resolveSafePath, h as handleConfigRequest, e as getContentType } from "../index-B_FwBmgt.mjs";
class MxCadAssetsWebpackPlugin {
  ...
  copyAssetsToOutput(compiler, logger, callback) {
    ...
    outputFs.mkdir(targetAssetsPath, { recursive: true }, (err) => {
      ...
      copyDir(this.sourceAssetsPath, targetAssetsPath, this.options, (err2) => {
        if (err2) {
...
function createStaticMiddleware(root, options, logger) {
  ...
  return async (req, res, next) => {
    ...
    const result = resolveSafePath(req.url, normalizedRoot);
    if (!result.success) {
